...
const INSTAGRAM_CONFIG = {
    ...
    // Fallback images if API fails or no token provided (Safety net)
    FALLBACK_IMAGES: [
        // Demo Video 1
        { 
...
function renderFeed(posts, track) {
    ...
    itemsToShow.forEach(post => {
        ...
        if (post.type === 'VIDEO') {
            // Play icon
            iconSvg = `<svg class="social-icon-lg" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>`;
        } else {
            // Instagram icon
        ...
        if (post.type === 'VIDEO' && post.videoUrl) {
            // Video: Click to play in place
            card.innerHTML = `
                <img src="${post.url}" alt="${post.caption ? post.caption.substring(0, 20) : 'Instagram Post'}" loading="lazy">
                <div class="social-overlay social-overlay--video" onclick="playInPlace(this, '${post.videoUrl}')" style="cursor: pointer;">
                    ${iconSvg}
                </div>
            `;
        } else {
